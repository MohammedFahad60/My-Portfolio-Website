...
const rings = [
  ...
  {
    rotation: [0.96, 0.35, 0.2] as [number, number, number],
    speed: 0.05,
    opacity: 0.3,
  },
  {
...
function Ring({
  ...
  index,
  reducedMotion,
}: (typeof rings)[number] & { index: number; reducedMotion: boolean }) {
  ...
  useFrame((state) => {
    ...
    ring.current.rotation.y =
      rotation[1] + Math.sin(state.clock.elapsedTime * 0.08 + index) * 0.04;
  });
  return (
    <mesh
      ref={ring}
      rotation={rotation}
      position={[0, 0, index === 1 ? 0.12 : index === 2 ? -0.16 : 0]}
      scale={0.9}
    >
